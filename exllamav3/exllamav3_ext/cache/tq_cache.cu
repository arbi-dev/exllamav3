...
)
{
    ...
    int padded_dim = next_pow2(head_dim);
    ...
    TORCH_CHECK(padded_dim <= TQ_MAX_HEAD_DIM, "padded head_dim must be <= ", TQ_MAX_HEAD_DIM);

    int bt = ((padded_dim + 31) / 32) * 32;
    int smem = (padded_dim + bt) * sizeof(float);

    // Grid: K and V in parallel (factor of 2)
    dim3 grid(seq_len * num_kv_heads * 2, 1, bsz);
    dim3 threads(bt);
    ...
    tq_encode_paged_kernel<<<grid, threads, smem, stream>>>(
        ...
        pages_per_seq, num_kv_heads, head_dim, padded_dim,
        k_packed_dim, v_packed_dim, k_bit_width, v_bit_width, seq_len,
        k_signs1.data_ptr<float>(), k_signs2.data_ptr<float>(),
        v_signs1.data_ptr<float>(), v_signs2.data_ptr<float>(),
        k_boundaries.data_ptr<float>(), v_boundaries.data_ptr<float>(),
...
)
{
    ...
    int bt = ((padded_dim + 31) / 32) * 32;
    int smem = padded_dim * sizeof(float);

    // Grid: K and V in parallel (factor of 2)
    dim3 grid(max_tokens * num_kv_heads * 2, 1, bsz);
    dim3 threads(bt);
    ...
    tq_decode_paged_kernel<<<grid, threads, smem, stream>>>(
        ...
        block_table.data_ptr<int32_t>(),
        pages_per_seq, num_kv_heads, head_dim, padded_dim,
        k_packed_dim, v_packed_dim, k_bit_width, v_bit_width,
        k_signs1.data_ptr<float>(), k_signs2.data_ptr<float>(),
        v_signs1.data_ptr<float>(), v_signs2.data_ptr<float>(),
